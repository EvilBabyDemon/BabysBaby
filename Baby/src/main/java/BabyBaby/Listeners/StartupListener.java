                ...
                try {
                    ...
                    while (rs.next()) {
                        try {
                            ...
                            remind.schedule(
                                    new GetReminder(event.getJDA().getUserById(rs.getString("USERID")),
                                            event.getJDA().getGuildById(rs.getString("GUILDID")), rs.getString("TEXTS"),
                                            rs.getString("CHANNELID"), rs.getString("pk")),
                                    (rs.getLong("TIME") - System.currentTimeMillis()) / 1000, TimeUnit.SECONDS);
                        } catch (Exception e) {
                            e.printStackTrace();
                ...
                try {
                    ...
                    while (rs.next()) {
                        try {
                            ...
                            e.printStackTrace();
                            System.out.println("Probably a User that left the server while being blinded. ID: "
                                    + rs.getString("USERID"));
                        }
                    }
                    stmt.close();
