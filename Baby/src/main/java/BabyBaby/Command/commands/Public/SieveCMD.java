...
public class SieveCMD implements IPublicCMD {
    ...
    public void handlePublic(CommandContext ctx) {
        ...
        if (cmd[0].equals(ctx.getGuild().getId())) {
            ...
            OffsetDateTime offDateTime = OffsetDateTime.of(Integer.parseInt(year), Integer.parseInt(month),
                    Integer.parseInt(day), 0, 0, 0, 0, ZoneOffset.UTC);
            for (Member mem : allMem) {
    ...
    public MessageEmbed getPublicHelp(String prefix) {
        ...
        embed.setDescription(
                "Command to find out who has the Role AmongUs but also the Anime role for example. Get all role id's with `"
                        + prefix + "allroles`. You can also input channelID's");
